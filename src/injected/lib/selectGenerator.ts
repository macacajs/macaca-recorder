...
import { InjectedScript } from './type';
...
export function generateSelector(
  ...
  try {
    ...
    const targetTokens = generateSelectorFor(
      injectedScript,
      targetElement,
      strict,
    );
    const bestTokens =
      targetTokens || cssFallback(injectedScript, targetElement, strict);
...
function filterRegexTokens(
  textCandidates: SelectorToken[][],
): SelectorToken[][] {
...
function generateSelectorFor(
  ...
  const calculate = (
    ...
    const noTextCandidates = buildCandidates(injectedScript, element).map(
      token => [token],
    );
...
function buildCandidates(
  ...
  for (const attribute of ['data-testid', 'data-test-id', 'data-test']) {
    if (element.getAttribute(attribute))
      candidates.push({
        engine: 'css',
        selector: `[${attribute}=${quoteAttributeValue(
  ...
  if (element.getAttribute('aria-label'))
    candidates.push({
      engine: 'css',
      selector: `[aria-label=${quoteAttributeValue(
  ...
  if (
    ...
    ['APPLET', 'AREA', 'IMG', 'INPUT'].includes(element.nodeName)
  )
    candidates.push({
      engine: 'css',
      selector: `${cssEscape(
        element.nodeName.toLowerCase(),
      )}[alt=${quoteAttributeValue(element.getAttribute('alt')!)}]`,
      score: 10,
    });
  ...
  if (element.getAttribute('role'))
    candidates.push({
      ...
      selector: `${cssEscape(
        element.nodeName.toLowerCase(),
      )}[role=${quoteAttributeValue(element.getAttribute('role')!)}]`,
      score: 50,
    });
  ...
  if (
    ...
    candidates.push({
      ...
      selector: `${cssEscape(
        element.nodeName.toLowerCase(),
      )}[name=${quoteAttributeValue(element.getAttribute('name')!)}]`,
      score: 50,
    });
  if (
    ...
    if (element.getAttribute('type'))
      candidates.push({
        ...
        selector: `${cssEscape(
          element.nodeName.toLowerCase(),
        )}[type=${quoteAttributeValue(element.getAttribute('type')!)}]`,
        score: 50,
      });
  }
    ...
    if (parent) {
      ...
      const token =
        sameTagSiblings.indexOf(element) === 0
          ? cssEscape(nodeName)
          : `${cssEscape(nodeName)}:nth-child(${
              1 + siblings.indexOf(element)
            })`;
      const selector = uniqueCSSSelector(token);
      if (selector) return makeStrict(selector);
